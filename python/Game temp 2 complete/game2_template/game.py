...
def print_menu(exits, room_items, inv_items):
    ...
    for item in room_items:
        print("TAKE " + item['name'].upper()+" to take a " + item['name'])
...
def execute_go(direction):
    ...
    if is_valid_exit(current_room["exits"],direction) == True:
        print("You are going to " + exit_leads_to(current_room["exits"],direction))
...
def execute_take(item_id):
    ...
    if item_id not in item_list:
        ...
        if cur_item not in current_room['items']:
            ...
            if carry_weight() + cur_item['mass'] > 3:
                print("You cannot carry anymore items, drop something.")
            else:
...
def main():
    ...
    if current_room == "reception":
        if "pen" in inventory:
            print("You have signed the register and won!")
